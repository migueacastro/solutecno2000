<script lang="ts">
	 ...
	 * con buscador centrado (⌘K/Ctrl+K) y a la derecha perfil, idioma y tema
	 * (sustituye al cluster flotante del layout). En móvil añade el hamburger
	 * que abre el drawer de la Sidebar.
	 */
	type Props = {
		profile: {
			id: string;
	...
	let { profile, onMenuToggle }: Props = $props();
