...
| `state` | CustomerState! | Enum: `DECLINED`, `DISABLED`, `ENABLED`, `INVITED` — solo relevante con cuentas *clásicas* |
| `locale` | String! | |
...
- **Roles** = definición de trabajo que empaqueta permisos granulares; un usuario puede tener **múltiples roles con permisos acumulativos**; categorías: Store, Organization, POS, Partner; existen roles predefinidos (ej. *Merchandiser*: Home + Products + Catalogs + Content).
